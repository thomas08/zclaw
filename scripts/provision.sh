...
Options:
  ...
  --cloud-url <url>         Cloud logging endpoint URL (optional)
  --cloud-key <token>       Cloud logging Bearer auth token (optional)
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --cloud-url)
            shift
            [ $# -gt 0 ] || { echo "Error: --cloud-url requires a value"; exit 1; }
            CLOUD_URL="$1"
            ;;
        --cloud-url=*)
            CLOUD_URL="${1#*=}"
        ...
        --cloud-key)
            shift
            [ $# -gt 0 ] || { echo "Error: --cloud-key requires a value"; exit 1; }
            CLOUD_KEY="$1"
            ;;
        --cloud-key=*)
            CLOUD_KEY="${1#*=}"
...

{
    ...
    if [ -n "$CLOUD_URL" ]; then
        printf "cloud_url,data,string,%s\n" "$(csv_escape "$CLOUD_URL")"
    fi
    if [ -n "$CLOUD_KEY" ]; then
        printf "cloud_key,data,string,%s\n" "$(csv_escape "$CLOUD_KEY")"
    fi
    if [ -n "$TG_CHAT_IDS" ]; then
