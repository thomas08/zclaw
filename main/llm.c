...
// ---------------------------------------------------------------------------
// Model auto-detection: query /v1/models and use whichever model is loaded.
// Called during llm_init() when no model is stored in NVS.
// ---------------------------------------------------------------------------
#if !CONFIG_ZCLAW_STUB_LLM && !CONFIG_ZCLAW_EMULATOR_LIVE_LLM

// Derive the /v1/models URL from the completions endpoint URL.
// e.g. http://192.168.1.40:1234/v1/chat/completions → http://192.168.1.40:1234/v1/models
static bool llm_derive_models_url(const char *api_url, char *out, size_t out_len)
{
    ...
    if (url_len > sfx_len &&
        strcmp(api_url + url_len - sfx_len, suffix) == 0) {
        size_t base = url_len - sfx_len;
        if (base + 7 + 1 > out_len) return false;
        memcpy(out, api_url, base);
        memcpy(out + base, "/models", 7);
        out[base + 7] = '\0';
        return true;
    }
    // Fallback: append /models
    int n = snprintf(out, out_len, "%s/models", api_url);
    return n > 0 && (size_t)n < out_len;
}

// HTTP GET /v1/models, parse first model ID into s_model.
// Returns true on success.
static bool llm_autodetect_model(const char *models_url)
{
#define MODELS_RESP_LEN 512
    char *buf = malloc(MODELS_RESP_LEN);
    if (!buf) return false;
    ...
    if (client && esp_http_client_open(client, 0) == ESP_OK) {
        ...
        int len = esp_http_client_read(client, buf, MODELS_RESP_LEN - 1);
        if (len > 0) {
            buf[len] = '\0';
            ...
            if (root) {
                ...
                if (cJSON_IsArray(data) && cJSON_GetArraySize(data) > 0) {
                    cJSON *id = cJSON_GetObjectItem(cJSON_GetArrayItem(data, 0), "id");
                    if (cJSON_IsString(id) && id->valuestring && id->valuestring[0]) {
                        strncpy(s_model, id->valuestring, sizeof(s_model) - 1);
...
esp_err_t llm_init(void)
{
    ...
    // Load custom API URL first (needed for model auto-detection below)
    s_api_url_override[0] = '\0';
    memory_get(NVS_KEY_LLM_API_URL, s_api_url_override, sizeof(s_api_url_override));

    // Load model; if absent, auto-detect from /v1/models (LM Studio / local servers)
    // or fall back to the backend default.
    s_model[0] = '\0';
    if (!memory_get(NVS_KEY_LLM_MODEL, s_model, sizeof(s_model)) || s_model[0] == '\0') {
        bool detected = false;
#if !CONFIG_ZCLAW_STUB_LLM && !CONFIG_ZCLAW_EMULATOR_LIVE_LLM
        if (s_api_url_override[0] != '\0') {
            char models_url[210];
            if (llm_derive_models_url(s_api_url_override, models_url, sizeof(models_url))) {
                ESP_LOGI(TAG, "No model set — querying %s", models_url);
                detected = llm_autodetect_model(models_url);
                if (detected) {
                    ESP_LOGI(TAG, "Auto-detected model: %s", s_model);
                }
            }
        }
#endif
